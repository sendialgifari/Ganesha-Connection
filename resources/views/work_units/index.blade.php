@seoTitle(__('Kelola Unit Kerja'))

<x-app-layout>
    <x-slot:header>
        <div class="flex justify-between align-items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Kelola Unit Kerja') }}
            </h2>
            <x-splade-link href="{{ route('work_units.create') }}"
                class="px-3 py-1 bg-green-500 hover:bg-green-600 text-white rounded-md">Tambah
                Unit Kerja
            </x-splade-link>
        </div>
        </x-slot>

        <div class="py-4">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <x-splade-table :for="$work_units" pagination-scroll="preserve" striped>
                    @cell('action', $work_unit)
                    <x-splade-link href="{{ route('work_units.edit', $work_unit) }}">
                        <x-splade-button class="font-bold bg-indigo-500 hover:bg-indigo-700 text-white">Edit</x-splade-button>
                    </x-splade-link>
                    <x-splade-form action="{{ route('work_units.destroy', $work_unit)}}" method="delete"
                        confirm="Delete Work Unit"
                        confirm-text="Are you sure you want to delete work unit?" confirm-button="Yes"
                        cancel-button="No">
                        <x-splade-button
                            class="font-bold bg-red-500 hover:bg-red-700 text-white">Hapus</x-splade-button>
                    </x-splade-form>
                    @endcell
                </x-splade-table>
            </div>
        </div>
</x-app-layout>