@seoTitle(__('Verifikasi Partner'))

<x-app-layout>
    <x-slot:header>
        <div class="flex justify-between align-items-center">
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                {{ __('Kelola Verifikasi Partner') }}
            </h2>
        </div>
        </x-slot>
        ...
        <div class="py-4">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <x-splade-table :for="$users" pagination-scroll="preserve" striped>
                    @cell('action', $user)
                    <x-splade-form action="{{ route('partner_approval_update', ['id' => $user->id])}}" method="put" confirm="Verify"
                        confirm-text="Are you sure you want to verify user?"
                        confirm-button="Yes" cancel-button="No">
                        <x-splade-button class="font-bold bg-indigo-500 hover:bg-indigo-700 text-white">Verify</x-splade-button>
                    </x-splade-form>
