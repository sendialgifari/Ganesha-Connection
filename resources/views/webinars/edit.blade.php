...
<x-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-splade-form class="space-y-4" action="{{ route('webinars.update', $webinar) }}" method="put"
                :default="$webinar">
                <x-splade-select name="webinar_category_id" label="Kategori produk" :options="$webinar_categories" />
    ...
    <script>
        $(document).ready(function() {
            @if($webinar->price_type == 1)
                $(".view_price").css("display", "none");
                $(".view_fake_price").css("display", "none");
            @else
            @if($webinar->fake_price == null || $webinar->fake_price == 0)
                $(".view_fake_price").css("display", "none");
            @else
